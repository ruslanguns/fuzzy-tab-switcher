  ...
  return (
    <>
      {keys.map((key, index) => {
        ...
        return (
          <Fragment key={`${prefix}-${key}-${occurrence}`}>
            <kbd class='pointer-events-none inline-flex h-5 items-center gap-1 rounded border border-border bg-muted px-1.5 font-mono opacity-100'>
              <span>{key}</span>
            </kbd>
    ...
    <div class='flex flex-col'>
      <div class='flex w-full items-center justify-between border-t border-border bg-secondary/50 px-3 py-2 font-mono text-[10px] text-muted-foreground select-none'>
        ...
        <button
          ...
          aria-label={`Switch theme. Current theme: ${themeLabel}`}
